...
pool.connect(function(err, client, done) {
    ...
    function insertEvents(agendaId, data, next){
        ...
        data.forEach(function(event){
            ...
            array.push( { 'start_time': event.start_time, 'end_time': event.end_time, 'name': event.name, 'more': more, 'agenda_id': agendaId, 'created_at': 'now()', 'updated_at': 'now()', 'rsvp_status': 'declined', 'event_type_id': 'facebook'} );
        });
        if(array.length!=0){
