    ...
    GET: {
        ...
        user_agendas: function(user_id, res){
            ...
            central.provider.query("SELECT * FROM user_agendas where user_id=$1", [user_id], function(err, result){
                ...
                var promises=[];
                result.rows.forEach(function(agenda){
                    var query = providers[agenda.provider].client.query("select id, $2::text as provider, name, editable, agenda_entity_id, agenda_type_id, more, active from agendas LEFT JOIN agenda_entities ON agendas.agenda_entity_id=entities.id where id =$1", [agenda.agenda_id, agenda.provider]);
                    promises.push(query);
                    query.then(function(){
                        providers[agenda.provider].done();
                    });
