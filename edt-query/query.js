            ...
            if(authenticated){
                central.provider.query("SELECT * FROM user_agendas where user_id=$1", [user_id], function(err, result){
                    ...
                    if(result.rows.length==0){
                        ...
                        central.provider.query("SELECT edt_id, first_name, last_name from users where edt_id=$1", [user_id], function(err, result){
                            central.done();
                            if(err) {
                                return console.error('error running query', err);
                            }
                    ...
                    if(result.rows.length==0){
                        ...
                        result.rows.forEach(function(anonymous_user){
                            if(anonymous_user.provider&&anonymous_user.agenda_id){
                                ...
                                promises.push(query);
                                query.then(function(){
                                    providers[anonymous_user.provider].done();
                                });
                            }
                        });
                        console.log("promises ready");
            ...
            FB.api('/me', { fields: ['id', 'email', 'first_name', 'last_name'] }, function (response) {
                ...
                jwt.verify(token, cert.pub, {algorithm: 'RS256'}, function(err, decoded) {
                    if (err) {
                        ...
                        central.provider.query("UPDATE users set facebook_id=$1, facebook_email=$2, is_validated=true, facebook_token=$3, ip_addr=$5 where edt_id=$4 RETURNING *", [response.id, response.email, facebook_token, id, ip_addr], function(err, result){
                            central.done();
